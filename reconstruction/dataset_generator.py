...
class ggxSurface:
    ...
    def norm_d(self, i, o):
        ...
        ag = self.ggx_ag
        cos_theta = dot(m, n)
        tan_thera = math.sqrt(1-cos_theta*cos_theta) / cos_theta
        if cos_theta <= 0:
            return 0
        else:
...
def gen_record(rid = 0):
    ...
    for i in range(5):
        imgs[i] = cv2.resize(imgs[i], (LGT_WIDTH // 2, LGT_HEIGHT // 2))
    ...
    record = {'id':rid, 'input':img.tolist(), 'output': imgs.tolist()}
...
def main():
    ...
    csvf = open('./ds/dataset.csv', 'a')
    writer = csv.writer(csvf)
    # writer.writerow(['filename'])
    for i in tqdm(range(100, 500)):
        rec = gen_record(i)
        with open('./ds/st_{:d}.json'.format(i), 'w') as f:
            json.dump(rec, f)
