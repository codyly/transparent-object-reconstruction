...
void createGeometry()
{
	...
	box["boxmin"]->setFloat(-30.0f, 0.0f, -50.0f);
	box["boxmax"]->setFloat(160.0f, 100.0f, 50.0f);
	...
	box_matl["phong_exp"]->setFloat(1000);
	...
	if (chull.get() || cube.get()) {
		...
		glass_matl["reflection_maxdepth"]->setInt(1);
	...
	if (cylinder.get()) {
		...
		glass_matl2["refraction_maxdepth"]->setInt(100);
...
void setupCamera()
{
	camera_eye = make_float3(30.0f, 49.0f, -0.0f);
	camera_lookat = make_float3(0.0f, 49.0f, 0.0f);
...
void setupLights()
{
	...
	BasicLight lights[] = {
		//{ make_float3(10.0f, 50.0f,-0.0f), make_float3(0.8f, 0.8f, 0.8f), 0.0f },
		//{ make_float3(10.0f, 50.0f,20.0f), make_float3(0.8f, 0.8f, 0.8f), 0.0f },
		{ make_float3(-0.0f, 80.0f,0.0f), make_float3(0.8f, 0.8f, 0.8f), 0.0f },
		{ make_float3(-0.0f, 20.0f,0.0f), make_float3(0.8f, 0.8f, 0.8f), 0.0f }
	};
...
void glutDisplay()
{
	...
	if (rotation) {
		if (t<= 200) {
			char buf[32];
			sprintf(buf, "%03d", t);
			const std::string outputImage = "./screenshots/set1/" + std::string(SAMPLE_NAME) + "_" + buf + ".ppm";
			std::cerr << "Saving current frame to '" << outputImage << "'\n";
			sutil::displayBufferPPM(outputImage.c_str(), getOutputBuffer());
		}
		t += 1;
		float angle = 0.01f * t * M_PIf;
